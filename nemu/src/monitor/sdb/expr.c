...
static bool make_token(char *e) {
  ...
  while (e[position] != '\0') {
	...
	for (i = 0; i < nr_token; i ++) {
 	 if (tokens[i].type == '*' && (tokens[i - 1].type == '+'||tokens[i - 1].type =='-'||tokens[i - 1].type =='*'||tokens[i - 1].type =='/'|| tokens[i-1].type == TK_EQ||tokens[i-1].type ==TK_NEQ||tokens[i-1].type ==TK_AND || tokens[i-1].type=='(' || i==0)) {
    		tokens[i].type = TK_P;
