...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, t = 4 + s->pc, s->dnpc = s->pc +imm ,R(dest)=t; IFDEF(CONFIG_FTRACE,pftrace(s->dnpc,s->pc)));
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(dest) = s->pc +4; s->dnpc = (src1+imm)&~1; IFDEF(CONFIG_FTRACE,pftrace(s->dnpc,s->pc)));
