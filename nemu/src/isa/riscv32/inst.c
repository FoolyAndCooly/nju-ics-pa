...
static int decode_exec(Decode *s) {
  int dest = 0;
  long long temp = 0;
  unsigned long long temp0 =0;
  word_t src1 = 0, src2 = 0, imm = 0,t=0;
  long long sr1, sr2;
  unsigned long long sr10,sr20;
  s->dnpc = s->snpc;
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(dest) = 4 + s->pc, s->dnpc = s->pc +imm ; IFDEF(CONFIG_FTRACE,pftrace(s->dnpc,s->pc)));
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, t = s->pc +4; s->dnpc = (src1+imm)&~1,R(dest)=t; IFDEF(CONFIG_FTRACE,pftrace(s->dnpc,s->pc)));
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(dest) = (((int )src1) >>(imm & 31) ) );
  ...
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, Mw(src1 +imm, 2, src2) );
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, if((int)src1 >= (int)src2) s->dnpc =s->pc+imm);
  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul    , R, R(dest) = src1 * src2);
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, sr1=(long long )(int)src1; sr2=(long long )(int)src2; temp=sr1*sr2;R(dest)=temp>>32);
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, sr10=(unsigned long long )src1; sr20=(unsigned long long )src2; temp0=sr10*sr20;R(dest)=temp0>>32);
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , I, R(dest) = src1 << (imm & 31) );
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, R(dest) = (uint32_t )src1 >> (imm & 31));
  ...
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret   , R,
   s->dnpc=cpu.csr.mepc;
  if ((cpu.csr.mstatus & 0x80) != 0) {cpu.csr.mstatus |= 0x8;}
  else{cpu.csr.mstatus &= ~0x8;}
